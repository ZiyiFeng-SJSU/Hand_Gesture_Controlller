...
face_landmark_node * stasm (char *face_id, int current_frame)
{
  ...
  static const char * const path = "./ORIGINAL-38.1.png";
  ...
  if (!img.empty())
  {
    ...
    if (stasm_search_single(&foundface, landmarks, (const char*)img.data, img.cols, img.rows, path, "/home/lanceb247/progs/hand_annotate/test/data"))
    {
      if (foundface)
      {
