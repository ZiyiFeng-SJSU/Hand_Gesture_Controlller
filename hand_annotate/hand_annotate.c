...
void display_form (char *face_id, int begin_frame, int end_frame, int current_frame, face_landmark_node *face_landmark_list_head, shape_file_node *shape_file_list_head)
{
  ...
  if (face_id != NULL)
  {
    printf ("    <div id=\"container\" style=\"width: 1920px; height: 1920px; border: 1px solid black; position: relative;\">\n");
...
//borkborkbork
  send_http_header();
  printf ("<p>filename: <b>%s</b></p>", filename);
  ...
  printf ("<p>filename: <b>%s</b></p> width: %d, height: %d", filename, img.cols, img.rows);

    if (stasm_search_single(&foundface, landmarks, (const char*)img.data, img.cols, img.rows, path, "/home/lanceb247/progs/hand_annotate/data"))
    {
      if (foundface)
      {
...
face_landmark_node * retrieve_data_from_annotated_face_landmark_data (PGconn *db_connection, char *face_id, int frame)
{
  ...
  strcat (db_statement, "'");
  strcat (db_statement, " ORDER BY frame, indice ASC");
  db_result = PQexec(db_connection, db_statement);
  if (PQresultStatus(db_result) == PGRES_TUPLES_OK)
  {
